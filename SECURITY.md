...
- **本地 `serve` 默认只绑 `127.0.0.1`**。Docker 镜像 CMD 为 `--host 0.0.0.0`，但
  docker-compose 的端口映射默认已收口为 `127.0.0.1:8787:8787`（无认证 API 不暴露给局域网）；
  直接 `docker run -p 8787:8787` 或需要局域网访问时，请自行加防火墙规则或反向代理认证。
